...
import (
	"context"
	redisPkg "finalai/internal/common/redis"
	...
	indexerConfig := &redisIndexer.IndexerConfig{
		...
		DocumentToHashes: func(ctx context.Context, doc *schema.Document) (*redisIndexer.Hashes, error) {
			...
			return &redisIndexer.Hashes{
				...
				Key: fmt.Sprintf("%s%s", redisPkg.GenerateIndexNamePrefix(filename), doc.ID),
...
func NewRAGQuery(ctx context.Context, username string) (*RAGQuery, error) {
	...
	filename, err := latestUploadedFilename(username)
	if err != nil {
		return nil, err
	}
...
func latestUploadedFilename(username string) (string, error) {
...
func LoadLatestUploadedDocument(username string) (*schema.Document, error) {
	filename, err := latestUploadedFilename(username)
	if err != nil {
		return nil, err
	}
	...
	return &schema.Document{
		ID:      fmt.Sprintf("fallback:%s", filename),
		Content: string(content),
		MetaData: map[string]any{
			"source": filePath,
		},
